...
export function AudienceStep({ campaignData, onUpdate }: AudienceStepProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Campaign Schedule — shown when audience is selected */}
      {(selectedSegment || (mode === 'upload' && uploadState?.status === 'ready')) && (
        <div className="flex flex-col gap-3">
          <div>
          ...
          <div className="flex gap-2">
            ...
            >
              <span className={['text-sm font-semibold', campaignData.schedule.type === 'one-time' ? 'text-cyan' : 'text-text-primary'].join(' ')}>
                One-time
              </span>
            ...
            <button
              type="button"
              onClick={() => {
                if (mode === 'upload') return; // recurring not allowed for custom uploads
                onUpdate({ schedule: { ...campaignData.schedule, type: 'recurring' } });
              }}
              disabled={mode === 'upload'}
              className={[
                'flex-1 rounded-lg border-2 px-4 py-3 text-left transition-all',
                mode === 'upload' ? 'cursor-not-allowed opacity-50 border-[#E5E7EB] bg-[#F9FAFB]' :
                campaignData.schedule.type === 'recurring'
                  ? 'border-cyan bg-cyan/5'
                  : 'border-[#E5E7EB] bg-white hover:border-[#D1D5DB]',
              ].join(' ')}
            >
              <span className={['text-sm font-semibold', campaignData.schedule.type === 'recurring' && mode !== 'upload' ? 'text-cyan' : 'text-text-primary'].join(' ')}>
                Recurring
              </span>
              <p className="mt-0.5 text-[11px] text-text-secondary">
                {mode === 'upload' ? 'Not available for custom uploads' : 'Run on a regular schedule'}
              </p>
            </button>
          </div>
          ...
          {/* Recurring config */}
          {campaignData.schedule.type === 'recurring' && mode !== 'upload' && (
            <div className="flex flex-wrap gap-3">
              <div className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) => onUpdate({ schedule: { ...campaignData.schedule, recurringFrequency: e.target.value as 'daily' | 'weekly' | 'biweekly' | 'monthly' } })}
                  className="rounded-lg border border-[#E5E7EB] bg-white px-3 py-2 text-sm text-text-primary focus:border-cyan focus:outline-none focus:ring-1 focus:ring-cyan"
                >
      ...
      {/* Campaign Budget — shown after schedule is visible */}
      {(selectedSegment || (mode === 'upload' && uploadState?.status === 'ready')) && (
        <div className="flex flex-col gap-1.5">
          <label htmlFor="campaign-budget" className="text-sm font-medium text-text-primary">
            {campaignData.schedule.type === 'recurring' ? 'Daily Budget' : 'Campaign Budget'}
          </label>
          <p className="text-xs text-text-secondary">
            {campaignData.schedule.type === 'recurring'
              ? 'Maximum spend per day across all channels. The platform will optimize spend within this daily limit.'
              : 'Total budget for this campaign run across all channels.'}
          </p>
          <div className="relative w-64">
            ...
            <input
              ...
              placeholder={campaignData.schedule.type === 'recurring' ? 'e.g., 50,000 or 50K' : 'e.g., 5,00,000 or 5L'}
              className="w-full rounded-lg border border-[#E5E7EB] bg-white py-2.5 pl-7 pr-3 text-sm text-text-primary placeholder:text-text-secondary focus:border-cyan focus:outline-none focus:ring-1 focus:ring-cyan"
            />
          </div>
        </div>
      )}
